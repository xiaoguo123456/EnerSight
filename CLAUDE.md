...
- ❌ 让本机记着的电站 id 永远失效 —— 换环境、目录下线、站被删都会让每个页面停在
  「站点不存在」；`STATION_NOT_FOUND` 要清掉 `stationStore.currentId` 退回默认站，见 05 §7.2
...
M3 卫星辐照实况改走 JAXA P-Tree 的葵花 SWR（免费；项目不盈利，已有账号，对外展示前须告知 P-Tree 事务局；
用 Buffalo 自建 Open-Meteo 自带的 `download-jaxa-himawari` 入库，不自己解析 NetCDF）；
