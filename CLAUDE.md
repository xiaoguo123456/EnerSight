...
| predictions/station 三模式 + history | ✅ | 默认 ECMWF / ICON / GFS 三家同算，主数字取中位那家；`/history` 读留档给预报演变；我的电站每日 08:30 签发（19 §一、§二） |
...
**规划中（docs/19）：** 三模式默认与预报演变（M1）、实测随手记与订正（M2）、
卫星辐照实况（M3，Open-Meteo 卫星辐射接口的 JAXA 产品，需 Professional 订阅）、气溶胶与积灰（M4）。
明确不做：51 成员集合与分位数、实测文件上传、两个细则考核。
