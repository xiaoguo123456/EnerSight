...
温度、风场、辐射改用 IFS HRES 空间 OM 文件；HTTP Range 读取原生纬带，按原生 O1280
行长度恢复经纬度，再重采样到图片覆盖范围。量化范围仅用于缓存，不是源数据分辨率。
缓存按起报批次、有效时刻、变量和区域隔离；解码及插值在线程中执行，避免阻塞 API。
微信仍使用 `addGroundOverlay`，不把标准 XYZ 模板当作小程序原生支持的能力。

复审约束：前端响应缓存按精确视野区分（数值标注和风矢量随视野改变）；后端图片仍按
区域复用。风粒子最近四点插值用单次扫描，不对完整矢量数组反复排序。HTTP Range
同时验证状态、字节长度与 Content-Range，拒绝错误区段，避免解码错误数据。
