...
| 数据 | TTL | 缓存键 |
| --- | --- | --- |
| 站点预报（实时天气、指数、趋势共用同一份 Forecast） | 8 小时 | 模型 + **起报时刻** + 0.1° 网格 |
| 15 分钟序列（只有单站 7 天预测要） | 8 小时 | 同上，按需拉取、单独计费 |
| 图层网格块 | 1 小时 | 模型 + 4° 块 |
| 卫星云图帧 | 10 分钟 | 全局 + 时间戳 |
...
**站点预报的缓存键带起报时刻，TTL 不再表示新鲜度。** 上游 6 小时才出一批，
而定时任务 15 分钟扫一轮：按时间片缓存会让每个网格每天回源 96 次、其中 92 次
拿到完全相同的数据。带上起报时刻后新批次一落地即刷新（可见延迟由元数据 TTL 决定，
5 分钟），同批次内永远命中，上游调用降到 1/24。TTL 8 小时只是内存回收的兜底。
