...
| 图层网格块 | 1 小时 | 模型 + 4° 块 |
| 卫星云图帧 | 10 分钟 | 全局 + 时间戳 |
| 省域卫星影像文件 | 48 小时后清理 | 省域 bbox + 观测时刻 + zoom |
...
| 图层 | 落地 |
| --- | --- |
| 云图 | Himawari 影像裁剪 → PNG；省域影像 JPEG → ground-overlay |
| 温度 | 网格插值 → 色阶填充 PNG |
| 辐射 | 网格插值 → 色阶填充 PNG |
...
**持久化的当前电站要能自愈。** 本机存着的电站 id 在服务端可能已经不存在 —— 换后端环境、
目录里下线、自建站被删。收到 `STATION_NOT_FOUND` 且失败的正是当前这座时，client 的 `onError`
会清掉本机记录（`stationStore.dropCurrent`），页面依赖 `currentId` 跟着变，自动改用默认电站重取。
不做这一步的话，每个按电站取数的页面都停在「站点不存在」，只剩一个永远失败的「重试」。
分享进来的别人的站 404 不动本机选择。
...
- 小程序：根据 bbox 自动选层级，一屏最多 12 张覆盖图，适配 `addGroundOverlay`；
  模拟器用同一组墨卡托图片预览。HRES 不再限制 24° 视野；云图默认模式仍限制 24°，
  首页省域入口的 `source=satellite` 模式另用一张真实卫星影像覆盖省域，最大 90°×70°。
