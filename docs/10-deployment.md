...
上线三模式与实测订正前，生产容器常驻 **1.07 GiB / 1.5 GiB**，余量不够：三模式让每次预测
算三份，每日 08:30 的签发任务还要给全部我的电站（94 座）各算三个模式。两处一起调：

| 项 | 原值 | 现值 | 理由 |
| --- | --- | --- | --- |
| `mem_limit` | 1536m | **2g** | 机器总 7.4 GB、可用 4.1 GB，还跑着另外十几个容器 |
| `issue_outlooks_concurrency` | 4 | **2** | 并发 4 时峰值十几份逐时数据同时在内存里；这个任务不赶时间 |

**上限本身不能去掉**：有 `mem_limit` 时撑爆只杀我们自己的容器，去掉就会波及同机的其他项目。
上线后盯两处：容器常驻内存，以及 08:30 那轮签发期间的峰值。
...
| 保留 | 脚本每轮删 **26 小时**没再写过的 `chunk_*.om`（两个域都清）。分块是 48 小时一个，上限约两块、7.4 GB |
| 出网 | 约 7.4 GB/天。套餐 7 TB/月，占比很小 |

为什么必须删：数据盘只剩 26 GB，而 `CACHE_SIZE` 设的是 32 GB（`cache.bin` 已 25 GB 且还在涨），
不清理会连自建气象服务一起拖垮。

**为什么是 26 小时而不是 6 小时**（2026-09-20 改）：分块按 48 小时切，换块后旧块里还留着
当天清晨那几小时。6 小时就删的话，隔一天会有一天，首页辐射曲线上的卫星实况从 08:00 才开始，
而不是从日出开始 —— 看着像 bug。26 小时保证换块后旧块再留一天，曲线永远从日出起。
清理覆盖 `jma_jaxa_himawari_70e_10min`（线上实时）与 `jma_jaxa_himawari_10min`（旧格点，
2018–2019 回测回填的历史帧），后者不清会一直堆在盘上。
