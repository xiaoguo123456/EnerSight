...
**保留策略**：删除粒度是 48 小时的块，删不到单帧。上一个块轮换后 **26 小时**删掉
（`find … -mmin +1560 -delete`），占用上限约两块 ≈ 7.4 GB。

一开始定的是 6 小时（只要「现在」，不展示历史）。加了辐射曲线上的卫星实况之后改成 26 小时：
块按 48 小时切，6 小时就删的话，隔一天会有一天曲线从 08:00 才开始而不是从日出开始，看着像 bug。
多留一天换来曲线永远完整，值这 3.7 GB。不能不删 —— 数据盘剩 26 GB，而 Open-Meteo 的 `CACHE_SIZE` 设的是 32 GB
...
- **用途**：
  ...
  1b. 首页与电站详情「24 小时气象趋势」的辐射曲线上叠一条绿色「卫星实况」（**已落地 2026-09-20**）：
      10 分钟的卫星值按「区间末的区间均值」重采样到曲线的时间轴（10 与 15 不整除，先摊到 5 分钟子区间再取平均，
      缺子区间就断线，不插值）；只有今天、只覆盖卫星还留着的那几小时。两条线的差就是今天预报偏了多少。
      **不要沿用功率图那块琥珀色底纹** —— 那表示「被限掉的部分」，预报与实况的差不是损失，阴天会整条涂满；
      `TrendChart` 的 `shortfallShade` 默认关，只有可发 / 上网那张图开；
