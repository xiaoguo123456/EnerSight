...
export const layersApi = {
  get: (layer: LayerType, bbox: BBox, zoom: number) => {
    ...
    const key = [useWeatherModel.getState().model, layer, span, Math.floor(bbox.west / span), Math.floor(bbox.south / span), Math.ceil(bbox.east / span), Math.ceil(bbox.north / span)].join(':')
    ...
    const value = api.get<LayerResponse>(`/v1/map/layers/${layer}`, {
      bbox: `${bbox.west},${bbox.south},${bbox.east},${bbox.north}`, zoom,
