...
import type { LayerResponse, LayerType, MapCloudHistoryResponse } from '@enersight/core/types'
...
export const layersApi = {
  history: () => api.get<MapCloudHistoryResponse>('/v1/map/layers/cloud/history'),
  get: (layer: LayerType, bbox: BBox, zoom: number, cloudMode: 'auto' | 'satellite' = 'auto', at?: string | null) => {
    ...
    const key = [model, layer, cloudMode, zoom, bbox.west, bbox.south, bbox.east, bbox.north, at ?? 'latest'].join(':')
    ...
    const value = api.get<LayerResponse>(`/v1/map/layers/${layer}`, {
      ...
      ...(layer === 'cloud' && cloudMode === 'satellite' ? { source: 'satellite' } : {}),
      ...(at ? { at } : {}),
