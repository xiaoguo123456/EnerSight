...
const PAGE_SIZE = 20
const TYPE_OPTIONS: { value: TypeFilter; label: string }[] = [{ value: 'all', label: '全部' }, { value: 'solar', label: '光伏' }, { value: 'wind', label: '风电' }]
const MINE_INFO = { title: '我的电站', content: '公开目录里没有的电站可以自己添加，仅本账号可见，最多 10 座。自建电站参与发电预测、预警、分析报告与卫星影像归档，不进入全目录汇总。今日电量由定时任务每小时更新，可填写限电规则得到预计上网电量。' }
...
function ownMetric(s: StationSummary) {
  const daily = s.metrics.daily_generation
  if (daily == null) return '今日估算稍后更新'
  const grid = s.metrics.grid_generation
  return grid == null ? `今日可发 ${thousands(daily, 0)} kWh` : `今日可发 ${thousands(daily, 0)} · 上网 ${thousands(grid, 0)} kWh`
}
...
export default function Stations() {
  ...
  // 自建电站：一次拉全量，本机按类型与关键词过滤；从表单返回时重新拉取。docs/17 §一
  ...
  useDidShow(() => {
    ...
    if (province) { Taro.removeStorageSync('enersight_prediction_province'); setKeyword(''); setScope('catalog'); setQuery({ ...INITIAL, province }) }
    const tab = Taro.getStorageSync('enersight_station_tab')
    if (tab === 'mine') { Taro.removeStorageSync('enersight_station_tab'); setScope('mine') }
  ...
  const loadMore = () => {
    if (scope !== 'catalog' || pending.current || result.loading || result.error || !result.hasMore) return
    pending.current = true
  ...
  const item = (s: StationSummary, own: boolean) => {
    const cap = formatPower(s.capacity)
    const limit = own ? curtailmentLabel(s) : null
    return <View key={s.id} className={`catalog__item ${currentId === s.id ? 'catalog__item--current' : ''}`} hoverClass="pressed" onClick={() => pick(s)}>
      ...
      <Text className="catalog__address">{s.address ? regionLabel(s.address) : own ? formatCoordinate(s.latitude, s.longitude) : regionLabel(null)}</Text>
      ...
      {own && <View className="catalog__own-row">
        <Text className="catalog__own-metric">{ownMetric(s)}</Text>
  ...
  return <View className="catalog">
    <View className="catalog__header">
      ...
      <View className="catalog__controls">
        ...
        {showTools && <View className="catalog__filters">
          ...
          {isMine
            ? <View className="catalog__add" hoverClass="pressed" onClick={addStation}><Icon name="plus" size={15} color="#1264d6" /><Text>添加电站</Text></View>
    ...
    {isMine && !loggedIn ? <EmptyState icon="user" title="登录后查看我的电站" actionText="登录" onAction={() => void requireLogin()} /> : isMine ? <>
      ...
      <View className="catalog__list">{result.items.map((s) => item(s, false))}</View>
