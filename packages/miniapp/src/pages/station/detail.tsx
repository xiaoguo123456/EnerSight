    ...
    <View className="detail">
      ...
      <View className="detail__body">
        ...
        <View className="detail__card">
          ...
          {station.is_own && station.type === 'wind' && <View className="detail__info-row"><Text className="detail__info-label">机型</Text><Text className="detail__info-value">{({ generic: '通用功率曲线', low_wind: '低风速机型', medium_wind: '中风速机型', high_wind: '高风速机型', custom: `自定义功率曲线 · ${station.power_curve?.length ?? 0} 点` } as Record<string, string>)[station.turbine_class ?? 'generic']}</Text></View>}
          {station.is_own && station.type === 'solar' && <View className="detail__info-row"><Text className="detail__info-label">安装方式</Text><Text className="detail__info-value">{station.mounting === 'single_axis' ? '单轴跟踪' : '固定支架'}{station.bifacial ? ' · 双面组件' : ''}</Text></View>}
