...
import { Icon, PageHeader, SegmentedTabs, Skeleton } from '@/components'
import { useStationStore } from '@/store'
import { decodeRouteParam } from '@/route'
import './form.scss'

/**
 * 自建场站：新建与编辑共用。docs/17 §一、§四
 * 经纬度三种来源：当前定位、地图选点（都是 GCJ-02）、手动输入（默认 WGS84，可选 GCJ-02）。
 * 提交带 coord，坐标转换由服务端完成，客户端不做换算。
 */
...
interface Form {
  ...
  latitude: string; longitude: string; coord: Coord
  tilt: string; azimuth: string; hub_height: string
  mode: 'none' | 'ratio' | 'schedule'; ratio: string; windows: WindowForm[]
}

const EMPTY: Form = { name: '', type: 'solar', capacity: '', latitude: '', longitude: '', coord: 'wgs84', tilt: '', azimuth: '', hub_height: '', mode: 'none', ratio: '', windows: [] }
...
const COORD_OPTIONS: { value: Coord; label: string; hint: string }[] = [
  { value: 'wgs84', label: 'WGS84', hint: 'GPS 设备、谷歌地图读数' }, { value: 'gcj02', label: 'GCJ-02', hint: '高德、腾讯、百度地图读数' },
]
...
function fromStation(s: StationSummary, form: Form): Form {
  ...
  return {
    ...
    latitude: s.latitude.toFixed(5), longitude: s.longitude.toFixed(5), coord: 'gcj02',
...
export default function StationForm() {
  ...
  useEffect(() => {
    ...
    }).catch(() => setFailed('站点加载失败')).finally(() => setLoading(false))
  }, [id, editing])
  ...
  const locate = async () => {
    ...
    try {
      ...
      patch({ latitude: r.latitude.toFixed(5), longitude: r.longitude.toFixed(5), coord: 'gcj02' })
      void Taro.showToast({ title: '已填入当前位置', icon: 'success' })
    } catch {
  ...
  const choose = async () => {
    try {
      ...
      patch({ latitude: r.latitude.toFixed(5), longitude: r.longitude.toFixed(5), coord: 'gcj02', ...(form.name ? {} : { name: r.name || '' }) })
    } catch { /* 用户取消 */ }
  }
  ...
  const remove = async () => {
    const answer = await Taro.showModal({ title: '删除场站', content: '将同时删除它的预警与发电记录，删除后可重新添加。', confirmText: '删除', confirmColor: '#ef4444' })
  ...
  const coordOption = COORD_OPTIONS.find((c) => c.value === form.coord)!

  return <View className="sform">
    <PageHeader title={editing ? '编辑场站' : '添加场站'} subtitle={editing ? form.name : '仅本账号可见'} />
    <View className="sform__body">
      ...
      {!loading && !failed && <>
        <View className="sform__card">
          <View className="sform__row-head"><Text className="sform__card-title">基本信息</Text><Text className="sform__hint">名称、类型、装机容量</Text></View>
          <View className="sform__field"><Text className="sform__label">场站名称</Text>
          ...
          <View className="sform__field"><Text className="sform__label">装机容量（kW，交流侧）</Text>
            <Input className="sform__input" type="digit" value={form.capacity} placeholder="例如 5000 表示 5 MW" placeholderClass="sform__ph" onInput={set('capacity')} /></View>
        </View>

        <View className="sform__card">
          <View className="sform__row-head"><Text className="sform__card-title">位置</Text><Text className="sform__hint">定位、地图选点或手动输入</Text></View>
          ...
          <View className="sform__coords">
            <View className="sform__field sform__field--half"><Text className="sform__label">纬度</Text>
              <Input className="sform__input" type="digit" value={form.latitude} placeholder="31.30000" placeholderClass="sform__ph" onInput={set('latitude')} /></View>
            <View className="sform__field sform__field--half"><Text className="sform__label">经度</Text>
              <Input className="sform__input" type="digit" value={form.longitude} placeholder="120.62000" placeholderClass="sform__ph" onInput={set('longitude')} /></View>
          </View>
          <Picker mode="selector" range={COORD_OPTIONS.map((c) => `${c.label} · ${c.hint}`)} value={COORD_OPTIONS.findIndex((c) => c.value === form.coord)}
            onChange={(e) => patch({ coord: COORD_OPTIONS[Number(e.detail.value)]!.value })}>
            <View className="sform__select"><Text className="sform__label">坐标系</Text><Text className="sform__select-value">{coordOption.label} · {coordOption.hint}</Text><Icon name="chevronDown" size={14} color="#64748b" /></View>
          </Picker>
          <View className="sform__note"><Icon name="helpCircle" size={12} color="#9ca3af" /><Text className="sform__note-text">定位与地图选点自动按 GCJ-02 提交；手动输入请按读数来源选坐标系，混用会让站点与云图错位几百米</Text></View>
        </View>

        <View className="sform__card">
          <View className="sform__row-head"><Text className="sform__card-title">出力模型参数</Text><Text className="sform__hint">不填用默认值</Text></View>
          {form.type === 'solar' ? <View className="sform__coords">
            ...
            <View className="sform__field sform__field--half"><Text className="sform__label">方位角（°）</Text>
              <Input className="sform__input" type="digit" value={form.azimuth} placeholder="默认 180 正南" placeholderClass="sform__ph" onInput={set('azimuth')} /></View>
        ...
        <View className="sform__card">
          <View className="sform__row-head"><Text className="sform__card-title">出力约束（限电 / 检修）</Text><Text className="sform__hint">可选</Text></View>
          ...
          {form.mode === 'ratio' && <View className="sform__field"><Text className="sform__label">限电比例（%）</Text>
            <Input className="sform__input" type="digit" value={form.ratio} placeholder="例如 15，表示全天出力按 85% 计" placeholderClass="sform__ph" onInput={set('ratio')} /></View>}
          {form.mode === 'schedule' && <>
            {form.windows.map((w, i) => <View key={i} className="sform__window">
              <View className="sform__window-head"><Text className="sform__label">时段 {i + 1}</Text>
            ...
            {form.windows.length < 12 && <View className="sform__add-window" hoverClass="pressed" onClick={() => patch({ windows: [...form.windows, { start: 11, end: 14, limit: '60', days: 'all' }] })}><Icon name="plus" size={14} color="#1264d6" /><Text>添加时段</Text></View>}
          </>}
          <View className="sform__note"><Icon name="helpCircle" size={12} color="#9ca3af" /><Text className="sform__note-text">约束只影响「预计上网电量」，可发电量与发电适宜度仍按气象条件估算；结束时刻早于开始表示跨零点</Text></View>
        </View>

        <View className={`sform__submit ${saving ? 'sform__submit--busy' : ''}`} hoverClass="pressed" onClick={saving ? undefined : submit}>
          <Text className="sform__submit-text">{saving ? '保存中…' : editing ? '保存修改' : '添加场站'}</Text>
        </View>
        {editing && <View className="sform__delete" hoverClass="pressed" onClick={remove}><Icon name="trash" size={14} color="#b91c1c" /><Text>删除场站</Text></View>}
        <Text className="sform__foot">自建场站仅本账号可见，参与预测、预警、报告与卫星归档，不进入全目录汇总。</Text>
      </>}
    </View>
  </View>
}
