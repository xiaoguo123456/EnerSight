...
const CONTENT: Record<string, { title: string; sections: [string, string][] }> = {
  help: { title: '使用指南与常见问题', sections: [
    ...
    ['如何反馈或纠错？', '在详情复制电站资料，再点击“资料纠错”；也可在「我的」页复制反馈信息后提交意见反馈，补充现象、时间和需要核对的字段。'],
  ] },
  sources: { title: '数据与计算说明', sections: [
    ...
    ['更新与时间', '当前气象与七天预报统一使用 15 分钟数据，同一模型批次共享缓存，预报按上游模型周期更新；卫星通常按 10 分钟帧序列获取，可能延迟或缺帧。当天报告访问时超过 10 分钟会重新生成。界面更新时间统一标注北京时间；24 小时趋势按电站当地日分段。目录更新时间是平台入库时间，不等于原始数据发布日。'],
  ...
  privacy: { title: '数据使用说明', sections: [
    ['登录与接口', '小程序通过微信静默登录进行身份校验，后端使用登录凭证保护接口访问；自建场站按微信账号隔离，其他用户不可见。不采集手机号、头像与昵称。'],
    ['自建场站信息', '你添加的场站名称、类型、坐标、装机容量、设备参数与限电规则保存在服务端，仅用于该场站的气象预测、预警、报告与卫星影像归档。删除场站会一并删除它的预警与发电记录。'],
    ['本机记录', '当前电站、最近浏览和常看收藏保存在本机，用于恢复查看位置。你可以在「我的」页清除最近浏览。'],
    ['位置信息', '仅在你添加自建场站时点击“当前定位”或“地图选点”才调用微信定位，用于填入场站坐标，不在后台获取位置；拒绝授权后仍可手动输入经纬度。地图的回位按钮回到当前电站，并非获取你的位置。'],
    ['微信隐私保护指引', '平台正式隐私保护指引可在「我的」页通过“微信隐私保护指引”打开。本页是当前功能的数据使用说明，具体授权以微信展示的指引为准。'],
  ] },
  about: { title: '关于晴川观象', sections: [
    ['晴川观象', '面向光伏与风电场景的公开电站、气象趋势与卫星观测工具。所有用户共享平台电站目录，选择即可查看；目录里没有的场站可以自己添加，仅本账号可见。'],
    ['使用建议', '先在电站目录查找目标电站，再查看气象、趋势与预警。发电分析为气象推算，请结合真实设备数据判断。'],
    ['反馈', '遇到数据或使用问题，可返回「我的」页使用微信“意见反馈”，并说明相关电站和发生时间。'],
  ] },
}
