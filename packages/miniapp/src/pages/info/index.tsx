...
const CONTENT: Record<string, { title: string; sections: [string, string][] }> = {
  help: { title: '使用指南与常见问题', sections: [
    ...
    ['如何反馈或纠错？', '在详情复制电站资料，再点击“资料纠错”；也可在「我的 → 帮助与反馈」复制反馈信息后提交意见反馈，补充现象、时间和需要核对的字段。'],
  ] },
  ...
  privacy: { title: '数据使用说明', sections: [
    ['登录', '浏览公开电站、地图、预警与全目录预测无需登录。添加和管理自建电站、导出数据前需通过微信登录，平台仅以微信分配的匿名标识区分账号，不采集手机号、头像与昵称。可在「我的」页退出登录。'],
    ['自建场站信息', '你添加的场站名称、类型、坐标、装机容量、设备参数与限电规则保存在服务端，仅本账号可见，用于该场站的气象预测、预警、报告与卫星影像归档。删除场站会一并删除它的预警、发电记录、分析报告与预测留档；在「我的 → 数据与隐私」页“删除我的数据”可一次删除本账号全部自建电站及上述记录，删除后不可恢复。'],
    ['本机记录', '当前电站、最近浏览和常看收藏保存在本机，用于恢复查看位置。你可以在「我的 → 数据与隐私」页清除最近浏览。'],
    ['位置信息', '仅在你添加自建场站时点击“当前定位”或“地图选点”才调用微信定位，用于填入场站坐标，不在后台获取位置；拒绝授权后仍可手动输入经纬度。地图的回位按钮回到当前电站，并非获取你的位置。'],
    ['微信隐私保护指引', '平台正式隐私保护指引可在「我的 → 数据与隐私」页通过“微信隐私保护指引”打开。本页是当前功能的数据使用说明，具体授权以微信展示的指引为准。'],
  ] },
  terms: { title: '用户服务协议', sections: [
    ['服务内容', '晴川观象提供公开电站目录、气象与卫星观测、发电量与出力的模型估算、预警与分析报告。浏览公开数据无需登录；添加自建电站与导出数据需要登录。'],
    ['账号', '通过微信登录，平台以微信分配的匿名标识区分账号，不采集手机号、头像与昵称。请勿将账号用于违法违规用途。'],
    ['自建电站与数据', '你填写的电站信息仅本账号可见，用于该电站的预测、预警与报告。你可以随时删除单个电站，或在「我的 → 数据与隐私」页删除全部数据，删除后不可恢复。'],
    ['估算性质', '发电量、功率、收益与预警均为基于气象预报的模型估算，不代表实际并网电量、结算金额或调度指令，请结合现场实测与专业判断使用。'],
    ['数据导出', '导出的 CSV 仅供你分析使用，引用时请注明为模型估算，并保留公开电站与气象数据的来源署名。'],
    ['协议变更', '平台可能调整功能或本协议，重大变更将在小程序内提示。'],
  ] },
  about: { title: '关于晴川观象', sections: [
    ...
    ['反馈', '遇到数据或使用问题，可进入「我的 → 帮助与反馈」使用微信“提交意见反馈”，并说明相关电站和发生时间。'],
  ] },
}
