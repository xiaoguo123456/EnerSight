  ...
  return <View className="fleet-history">
    ...
    {req.status === 'error' ? <ErrorState error={req.error.status === 404 ? new ApiError('HISTORY_UNAVAILABLE', '历史记录服务暂未开放，请稍后再试。', 404) : req.error} onRetry={req.reload} /> : !d ? <Skeleton height={200} /> : <>
      <View className="home__card">
        ...
        {!d.recorded_days ? <View className="history-empty">暂无记录</View> : <>
          <View className="history-legend"><View className="history-legend__item"><View className="history-legend__swatch history-solar" /><Text>光伏</Text></View><View className="history-legend__item"><View className="history-legend__swatch history-wind" /><Text>风电</Text></View></View>
